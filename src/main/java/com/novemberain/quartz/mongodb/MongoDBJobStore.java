...
public class MongoDBJobStore implements JobStore, Constants {
	...
	public boolean removeTrigger(TriggerKey triggerKey) throws JobPersistenceException {
		...
		if (triggers.count() > 0) {
			...
			if (trigger.containsField(TRIGGER_JOB_ID)) {
				...
				// Remove the orphaned job if it's durable and has no other
	...
	public List<TriggerFiredResult> triggersFired(List<OperableTrigger> triggers) throws JobPersistenceException {
		...
		for (OperableTrigger trigger : triggers) {
			...
			TriggerFiredBundle bndle = new TriggerFiredBundle(retrieveJob(trigger), trigger, cal, false, new Date(), trigger.getPreviousFireTime(), prevFireTime, trigger.getNextFireTime());
	...
	private DB selectDatabase(Mongo mongo) {
		...
		// MongoDB defaults are insane, set a reasonable write concern
		// explicitly. MK.
	...
	protected ObjectId storeJobInMongo(JobDetail newJob, boolean replaceExisting) throws ObjectAlreadyExistsException {
		...
		DBObject existingJob = jobCollection.findOne(keyToDBObject(key));
		...
		try {
			if (replaceExisting && existingJob != null) {
				jobCollection.update(existingJob, job);
			} else {
	...
	private Collection<ObjectId> idsFrom(Collection<DBObject> docs) {
		// so much repetitive code would be gone if Java collections just had
		// .map and .filter…
